...
class OBSWebsocket:
    ...
    def on_replay_buffer_saved(self, event_data):
        # self.app.config['OBS_DROP_REPLAY'] = True
        for camera in self.app.config['CAMERAS']:
            print(camera)
            thread = self.socketio.start_background_task(self.app.config['CAMERAS'][camera].save_replay)
    ...
    def set_config_cameras_recording_status(self, app):
        replay_buffer_status = app.config['OBS_REPLAY_BUFFER_STATUS']
        if replay_buffer_status:
    ...
    def toggle_virtual_cams_state(self, app):
        ...
        for camera in app.config['CAMERAS']:
            if app.config['OBS_REPLAY_BUFFER_STATUS']:
    ...
    def get_scene_name_by_filter_name(self, filter_name):
        ...
        for source in scene_sources['sceneItems']:
            ...
            for filter in source_filters:
                if filter_name == filter['filterName']:
